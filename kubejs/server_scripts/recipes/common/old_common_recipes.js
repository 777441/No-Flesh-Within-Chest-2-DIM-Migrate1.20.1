// priority: 800
// 旧版 recipes/common.js 迁移 — 通用合成/烧炼配方
...
ServerEvents.recipes(event => {
    ...
    event.shaped(Item.of('extraarmor:copper_helmet', 1), ['LOL', 'L L'], { L: 'minecraft:copper_ingot', O: 'minecraft:raw_copper' })
    event.shaped(Item.of('extraarmor:copper_chestplate', 1), ['L L', 'LOL', 'LLL'], { L: 'minecraft:copper_ingot', O: 'minecraft:raw_copper' })
    event.shaped(Item.of('extraarmor:copper_leggings', 1), ['LOL', 'L L', 'L L'], { L: 'minecraft:copper_ingot', O: 'minecraft:raw_copper' })
    event.shaped(Item.of('extraarmor:copper_boots', 1), ['O O', 'L L'], { L: 'minecraft:copper_ingot', O: 'minecraft:raw_copper' })

    event.shaped(Item.of('kubejs:organ_recycler', 1), ['OOO', 'OCO', 'OMO'], { C: '#kubejs:organ', O: 'lightmanscurrency:coin_iron', M: 'wares:delivery_table' })
    ...
    event.shaped('irons_spellbooks:scroll', [
        ...
        .modifyResult((grid, stack) => {
            ...
            if (!scroll.nbt.ISB_Spells || !scroll.nbt.ISB_Spells.data || !scroll.nbt.ISB_Spells.data[0]) return
            let curScrollLevel = scroll.nbt.ISB_Spells.data[0].getInt('level') + 1
            if (curScrollLevel >= 15) return
    ...
    event.shapeless('kubejs:paradise_regained', ['kubejs:god_consciousness', 'kubejs:god_consciousness', 'kubejs:god_consciousness'])
        .modifyResult((grid, stack) => {
            ...
            let m1 = nbt1 && nbt1.mobType, m2 = nbt2 && nbt2.mobType, m3 = nbt3 && nbt3.mobType
            if ((m1 != m2) && (m2 != m3) && (m3 != m1)) {
                return Item.of('kubejs:paradise_regained')
            }
            return stack
        })
    ...
    event.shapeless('kubejs:infinity_force', ['kubejs:infinity_force', 'kubejs:infinity_force'])
        .modifyResult((grid, stack) => {
            ...
            let ft1 = (item1.nbt && item1.nbt.forgeTimes) || 0
            let ft2 = (item2.nbt && item2.nbt.forgeTimes) || 0
            if (ft1 == ft2) {
                let forgeTimes = ft1
    ...
    event.shapeless(Item.of('kubejs:mosquito_repellent'), ['irons_spellbooks:magic_cloth', 'minecraft:rotten_flesh'])
    event.shapeless('kubejs:sausage_casing', ['#kubejs:intestine'])

    event.shapeless(Item.of('kubejs:appendix').withName(Text.gray(Text.translatable("kubejs.recipe.tip.1"))), [Ingredient.of(['@chestcavity', '#kubejs:organ']), 'biomancy:healing_additive'])
        .modifyResult((grid, stack) => {
            for (let i = 0; i <= 9; i++) {
