...
ServerEvents.recipes(event => {
    event.shapeless('kubejs:organ_charm', ['kubejs:empty_organ_charm', '#kubejs:organ'])
        .modifyResult((grid, stack) => {
            ...
            if (global.organCharmNbtMap[organ.id]) stack.nbt.merge(global.organCharmNbtMap[organ.id])
    ...
    event.shapeless('chestcavity:appendix', ['kubejs:organ_charm'])
        ...
        .modifyResult((grid, stack) => {
            ...
            if (charm.nbt && charm.nbt.organ) stack = Item.of(charm.nbt.organ.id, charm.nbt.organ.tag)
            return stack;
        })
})
