// priority: 500
// 旧版 common/loot.js + 新版基础掉落合并
// 包含: Boss掉落/宝箱战利品/基础世界掉落
LootJS.modifiers(event => {
    // ============ 新版基础掉落（来自旧 common_loot.js） ============
    ...
    bossTypeList.forEach(bossType => {
        event.addEntityLootModifier(bossType)
            ...
            .apply(ctx => {
                ...
                if (diffLevelNum >= 4) {
                    ctx.addLoot(LootEntry.of('kubejs:dark_stardust_fragment').when((c) => c.randomChance(0.8)))
                    ctx.addLoot(LootEntry.of('kubejs:unbreakable_core').when((c) => c.randomChance(0.05)))
                    ctx.addLoot(LootEntry.of('kubejs:disenchantment_book').when((c) => c.randomChance(0.2)))
                }
            })
    })
    ...
    event.addEntityLootModifier("minecraft:villager")
        .apply(ctx => {
            if (Math.random() < 0.03) ctx.addLoot(getRandomPotionWares())
            if (Math.random() < 0.005) ctx.addLoot(getRandomChallengeWares())
            if (Math.random() < 0.012) ctx.addLoot(getRandomEggWares())
            if (Math.random() < 0.018) ctx.addLoot(getRandomSpecialWares())
            if (Math.random() < 0.008) ctx.addLoot(getRandomOrganWares())
            if (Math.random() < 0.03) ctx.addLoot(getRandomOreWares())
        })
    ...
    event.addLootTypeModifier(LootType.CHEST)
        ...
        .apply(ctx => {
            if (Math.random() < 0.12) ctx.addLoot(getRandomPotionWares())
            if (Math.random() < 0.05) ctx.addLoot(getRandomChallengeWares())
            if (Math.random() < 0.02) ctx.addLoot(getRandomEggWares())
            if (Math.random() < 0.08) ctx.addLoot(getRandomSpecialWares())
            if (Math.random() < 0.03) ctx.addLoot(getRandomOrganWares())
            if (Math.random() < 0.07) ctx.addLoot(getRandomOreWares())
        })
