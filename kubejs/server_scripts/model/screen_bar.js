// priority: 1000
// 旧版 utils/screen_bar.js 迁移 — 单片镜HUD
function initAllBar(player) {
    if (!player.persistentData.contains(resourceCount)) player.persistentData.putInt(resourceCount, 0)
    if (!player.persistentData.contains(resourceCountMax)) player.persistentData.putInt(resourceCountMax, defaultResourceMax)
    if (!player.persistentData.contains(warpCount)) player.persistentData.putInt(warpCount, 0)
    if (!player.persistentData.contains(warpCountMax)) player.persistentData.putInt(warpCountMax, defaultWarpMax)
    ...
    let visible = checkCurios(player, 'kubejs:archivist_eyeglass')
    player.paint({
        barBackGround: { type: 'rectangle', x: 11, y: '-$screenH/2+49', w: 22, h: 101, alignX: 'left', alignY: 'bottom', texture: 'kubejs:textures/gui/resource_bar.png', visible: visible },
        resourceBarOverlay: { type: 'rectangle', x: 11, y: '-$screenH/2+49', v0: 1 - resourcePercent, v1: 1, w: 11, h: 101 * resourcePercent, alignX: 'left', alignY: 'bottom', texture: 'kubejs:textures/gui/resource_bar_overlay.png', visible: visible },
        warpBarOverlay: { type: 'rectangle', x: 22, y: '-$screenH/2+49', v0: 1 - warpPercent, v1: 1, w: 11, h: 101 * warpPercent, alignX: 'left', alignY: 'bottom', texture: 'kubejs:textures/gui/warp_bar_overlay.png', visible: visible },
        barCountText: { type: 'text', x: 13, y: '-$screenH/2+59', text: `§6${resourceCountNum}§f/§5${warpCountNum}§f`, alignX: 'left', alignY: 'bottom', visible: visible }
    })
    if (visible) Utils.server.scheduleInTicks(100, event => { player.paint({ barCountText: { visible: false } }) })
}
...
function updateResourceMaxCount(player, maxCount) {
    ...
    if (count > maxCount) { count = maxCount; player.persistentData.putInt(resourceCount, count) }
...
function updateWarpMaxCount(player, maxCount) {
    ...
    if (count > maxCount) { count = maxCount; player.persistentData.putInt(warpCount, count) }
...
function checkCurios(player, itemId) {
    ...
    for (let typeName of curios.keySet()) {
        ...
        for (let slot = 0; slot < slotHandler.getSlots(); slot++) {
            let item = slotHandler.getStackInSlot(slot)
            if (item && !item.isEmpty() && item.id == itemId) return true
        }
    }
    return false
}
...
function updateSideBar(player, visible) {
    ...
    if (curResource > maxResource) { player.persistentData.putInt(resourceCount, maxResource); curResource = maxResource }
    let curWarp = player.persistentData.getInt(warpCount)
    let maxWarp = player.persistentData.getInt(warpCountMax)
    if (curWarp > maxWarp) { player.persistentData.putInt(warpCount, maxWarp); curWarp = maxWarp }
